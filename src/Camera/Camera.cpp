...
namespace RayTracer {
    ...
    void Camera::rotate(const Math::Vector3D& rotation)
    {
        double yaw = rotation.getY() * M_PI / 180.0;
        double pitch = rotation.getX() * M_PI / 180.0;

        Math::Vector3D right = direction.cross(up).normalize();

        Math::Vector3D newDir = direction * cos(pitch) + up * sin(pitch);
        newDir = newDir.normalize();
        up = right.cross(newDir).normalize();

        direction = newDir * cos(yaw) + right * sin(yaw);
